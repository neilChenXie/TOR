...
/*stage5*/
char router_ip[20]; //for different router remenber its own eth_ip
uint16_t pre_port;  //port of incoming packet
...
int rand_hop(int *group) {
	...
	for(i = 0; i < num_router; i++) {
		...
		//printf("!!!the %dth position is: %d!!!!!!!!!!!", i, pos_ary[i]);
	}
	/*swap pos*/
	for(i = 0; i < num_router; i++) {
		temp = group[i];
